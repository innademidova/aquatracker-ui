...
export const waterApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getMonthlyWaterConsumption: builder.query<
      ...
      GetMonthlyWaterConsumptionRequest
    >({
      query: (getMonthlyWaterConsumptionRequest) => ({
        url: `Water/monthly-consumption?year=${getMonthlyWaterConsumptionRequest.year}&month=${getMonthlyWaterConsumptionRequest.month}`,
      }),
    }),
  }),
});
...
export const {
  ...
  useGetMonthlyWaterConsumptionQuery,
