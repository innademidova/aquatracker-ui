...
import { getDailyConsumptionAmount } from '@/utils/waterInfoHelper';
...
const WaterConsumptionChart: FC<WaterConsumptionChartProps> = ({ days, currentDate, monthlyWaterConsumption }) => {
    ...
    const mapDaysToData = (): DataPoint[] => {
        return days.map(el => ({
            day: el,
            amount: getDailyConsumptionAmount(getFullDate(currentDate, el), monthlyWaterConsumption) / 1000,
        } as DataPoint));
    }
